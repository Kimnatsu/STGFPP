...
(function () {
  ...
  function renderList() {
    document.getElementById('csListSection').hidden = false;
    document.getElementById('csDetail').hidden = true;
  ...
  function renderDetail(id) {
    ...
    if (!f) return;
    document.getElementById('csListSection').hidden = true;
    var detail = document.getElementById('csDetail');
    detail.hidden = false;
    document.getElementById('csTitle').innerHTML =
      (f.category ? '<span class="badge badge--patch">' + UI.esc(f.category) + '</span> ' : '') + UI.esc(f.title);
    document.getElementById('csMeta').textContent = f.author + ' · ' + UI.fmtDate(f.date);
    document.getElementById('csContent').innerHTML = UI.renderContent(f.content);
    /* 본문 하단: 고객센터 목록형 Box (현재 목록 데이터) */
    document.getElementById('csList').innerHTML = '<ul class="lst">' + filtered().slice(0, 6).map(function (x) {
  ...
  /* ================= 부팅 ================= */
  function start() {
    UI.fillBanner(document.getElementById('csBannerMedia'), null, []);
    UI.skelRows(document.getElementById('csGrid'), 5);
    ...
    document.getElementById('csAsk').addEventListener('click', askModal);
    document.getElementById('csMyBtn').addEventListener('click', myModal);
    document.getElementById('goCsList').addEventListener('click', function () { renderList(); });

    loadAll().then(function () {
      /* 고객센터 전용 배너 (banners 중 page=cs 우선, 없으면 첫 배너) */
      return FB.getBanners().then(function (bs) {
        var csB = bs.filter(function (b) { return String(b.page || '').toLowerCase().indexOf('cs') > -1 || String(b.page || '').toLowerCase().indexOf('service') > -1; });
        UI.fillBanner(document.getElementById('csBannerMedia'), null, csB.length ? csB : bs.slice(0, 1));
      });
    }).then(function () {
      renderList();
    }).catch(function (e) {
      UI.toast(FB.errMsg(e) + ' — 고객센터 데이터를 불러오지 못했습니다.', 'err');
      UI.empty(document.getElementById('csGrid'), { title: '고객센터 정보를 불러오지 못했습니다.', desc: '네트워크 또는 Firebase 연결을 확인해 주세요.' });
    });
  }
